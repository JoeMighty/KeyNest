...
export default function StampDutyPage() {
  ...
  const downloadPDF = () => {
    ...
    doc.setFontSize(22);
    doc.text("Stamp Duty Calculation (UK)", 20, 30);
    
    doc.setFontSize(12);
    doc.setFont("helvetica", "normal");
    doc.text(`Property Price: ${formatCurrency(price)}`, 20, 45);
    doc.text(`Buyer Status: ${buyerType.replace("-", " ")}`, 20, 52);
    
    doc.setFont("helvetica", "bold");
    doc.setFontSize(16);
    doc.text(`Total Stamp Duty: ${formatCurrency(result.totalTax)}`, 20, 65);
    doc.text(`Effective Tax Rate: ${result.effectiveRate.toFixed(2)}%`, 20, 75);
    ...
    doc.save(`Stamp_Duty_Calculation_${price}.pdf`);
              ...
              <Card className="overflow-hidden border-2 border-primary/20 shadow-xl shadow-primary/5">
                ...
                <div className="bg-muted/50 p-6 border-t flex flex-wrap gap-4 items-center justify-center md:justify-start">
                  <Button className="gap-2 px-8 rounded-xl h-12 shadow-lg shadow-primary/10" onClick={downloadPDF}>
                    <Download className="w-4 h-4" /> Download PDF Breakdown
                  </Button>
