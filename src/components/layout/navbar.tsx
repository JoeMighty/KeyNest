  ...
  return (
    <>
      <nav className={cn(
        ...
        isHome ? "fixed top-0" : "sticky top-0 bg-background border-b border-border/40",
        scrolled 
          ? "border-b border-slate-200/50 dark:border-slate-800/50 bg-white/80 dark:bg-slate-950/80 backdrop-blur-xl h-16 shadow-sm" 
          : cn("h-24", isHome ? "bg-transparent border-transparent" : "bg-background")
      )}>
        ...
        {isOpen && (
          <div className={cn(
            "md:hidden fixed inset-0 bg-slate-950/95 backdrop-blur-2xl px-6 py-10 space-y-4 z-50 border-t border-white/10 animate-in fade-in slide-in-from-top-4 duration-300",
            scrolled ? "top-[64px]" : "top-[96px]"
          )}>
            {navItems.map((item) => (
              <Link
                key={item.href}
